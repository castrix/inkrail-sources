...
The two `example-*` packages are synthetic demos, not website catalogs. The dedicated adult adapters are excluded from this public repository. MangaDex is classified as a non-adult source and appears without enabling adult-source visibility. Its maximum content rating setting controls which titles are returned. See [MangaDex configuration](../packages/mangadex/README.md).
